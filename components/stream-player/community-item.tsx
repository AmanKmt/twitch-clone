...
export const CommunityItem = ({ hostName, viewerName, participantName, participantIdentity }: CommunityItemProps) => {
    ...
    const handleBlock = () => {
        ...
        startTransition(() => {
            onBlock(participantIdentity).then(() => {
                toast.success(`${participantName} blocked`);
            }).catch(() => {
    ...
    return (
        <div className={cn("flex group items-center justify-between w-full p-2 rounded-md text-sm hover:bg-white/5", isPending && "opacity-50 pointer-events-none")}>
            <p style={{ color: color }}>
                {participantName}
            </p>
